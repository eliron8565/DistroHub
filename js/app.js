class DistroHubApp {
  ...
  bindUI(){
    ...
    this.on('modalClose','click',()=>this.closeModal());this.on('authClose','click',()=>this.closeAuth());this.on('accountBtn','click',()=>this.user?this.signOut():this.openAuth());this.on('loginFromFav','click',()=>this.openAuth());this.on('authSwitch','click',()=>this.switchAuth());this.on('authForm','submit',e=>this.authSubmit(e));
  ...
  applyFilters(){
    ...
    // Safety fallback: never leave a populated catalog visually empty because of malformed metadata/filter state.
  ...
  async initAuth(){if(!window.DistroHubSupabase)return;try{const sb=await DistroHubSupabase.loadClient();if(!sb)return;const {data}=await sb.auth.getUser();this.setUser(data?.user||null);sb.auth.onAuthStateChange((_e,s)=>this.setUser(s?.user||null))}catch(e){console.warn('[DistroHub] Auth unavailable',e)}}
