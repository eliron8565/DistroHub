...
(() => {
  ...
  function installButton(btn,app){if(!btn||btn.dataset.surpriseInstalled)return;const fresh=btn.cloneNode(true);btn.replaceWith(fresh);fresh.dataset.surpriseInstalled='true';fresh.addEventListener('click',()=>{const pool=(app.systems||[]).filter(s=>s&&s.id);if(!pool.length){app.toast?.('The OS catalog is still loading.');return}const previous=app.lastSurpriseId;const choices=pool.length>1&&previous?pool.filter(s=>s.id!==previous):pool;const picked=choices[Math.floor(Math.random()*choices.length)];app.lastSurpriseId=picked.id;fresh.disabled=true;fresh.classList.add('surprising');fresh.innerHTML='<i class="fa-solid fa-dice-d20 fa-spin"></i> Rolling...';document.getElementById('systemGrid')?.classList.add('surprise-pulse');window.setTimeout(()=>document.getElementById('systemGrid')?.classList.remove('surprise-pulse'),700);window.setTimeout(()=>{fresh.disabled=false;fresh.classList.remove('surprising');fresh.innerHTML='<i class="fa-solid fa-shuffle"></i> Surprise me';app.open(picked)},650)})}
...
// Load the new OSNova experience without changing the existing HTML script order.
(() => {
  ...
  const boot=()=>{load('js/rebrand.js');load('js/feature-pack.js')};
